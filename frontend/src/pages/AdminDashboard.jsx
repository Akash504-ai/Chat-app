...
const AdminDashboard = () => {
  ...
  // --- 🚀 FIX: GENERATE REPORT (CSV Export Logic) ---
  ...
  const formatGrowthData = () => {
    ...
    dashboardStats.monthlyUsers?.forEach((item) => {
      userMap[item._id] = item.users;
    });
    dashboardStats.monthlyMessages?.forEach((item) => {
      messageMap[item._id] = item.messages;
    });
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
        ...
        <StatCard
          ...
          loading={loading}
          isWarning={dashboardStats?.pendingReports > 0}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-xs opacity-50">
                Monthly active users vs messages
              </p>
            </div>
            {/* 🚀 FIX: VIEW DETAILS (Modal Trigger) */}
            <button
        ...
        <div className="card bg-base-100 border border-base-300 shadow-sm p-6">
          ...
          <p className="text-xs opacity-50 mb-6">
            Breakdown of platform activity
          </p>
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={[
                    { name: "Users", value: dashboardStats?.totalUsers || 1 },
                    { name: "Groups", value: dashboardStats?.totalGroups || 1 },
                    {
                      name: "Reports",
                      value: dashboardStats?.totalReports || 1,
                    },
                  ]}
                  innerRadius={60}
                  outerRadius={80}
          ...
          <div className="space-y-3 mt-4">
            <div className="flex items-center justify-between text-sm p-2 bg-base-200/50 rounded-lg">
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-blue-500"></div>
                <span className="opacity-70 font-medium">Users</span>
              </div>
              <span className="font-bold">{dashboardStats?.totalUsers}</span>
            </div>
            <div className="flex items-center justify-between text-sm p-2 bg-base-200/50 rounded-lg">
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-purple-500"></div>
                <span className="opacity-70 font-medium">Groups</span>
              </div>
              <span className="font-bold">{dashboardStats?.totalGroups}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {showDetails && (
        <div className="modal modal-open backdrop-blur-md transition-all duration-500">
          <div className="modal-box max-w-3xl bg-base-100/95 shadow-2xl border border-base-300 rounded-[2.5rem] p-0 overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <div className="relative bg-gradient-to-br from-primary/20 via-base-100 to-base-100 px-8 py-10 border-b border-base-300">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-5">
                  <div className="p-4 bg-primary text-primary-content rounded-2xl shadow-2xl shadow-primary/40 rotate-3 group-hover:rotate-0 transition-transform">
                    <TrendingUp size={28} strokeWidth={2.5} />
                  </div>
                  <div>
                    <h3 className="font-black text-3xl tracking-tight text-base-content">
                      Growth Analytics
                    </h3>
                    <p className="text-sm font-medium opacity-60 flex items-center gap-2">
                      <Clock size={14} /> Monthly performance metrics for{" "}
                ...
                <button
                  ...
                  className="btn btn-circle btn-ghost bg-base-200/50 hover:bg-error/20 hover:text-error transition-all"
                >
                  <X size={24} />
                </button>
              </div>
            ...
            <div className="p-8 max-h-[60vh] overflow-y-auto scrollbar-thin scrollbar-thumb-base-300">
              <table className="table w-full border-separate border-spacing-y-3">
                <thead>
                  <tr className="text-base-content/40 border-none">
                    <th className="bg-transparent font-black uppercase text-[11px] tracking-widest pl-4">
                      Reporting Period
                    </th>
                    <th className="bg-transparent font-black uppercase text-[11px] tracking-widest text-center">
                      Acquisition
                    </th>
                    <th className="bg-transparent font-black uppercase text-[11px] tracking-widest text-right pr-4">
                      Engagement
                    </th>
                  </tr>
                </thead>
                      ...
                      <td className="bg-base-200/40 group-hover:bg-primary/10 text-center transition-colors">
                        <div className="flex flex-col items-center">
                          ...
                          <span className="text-[10px] font-bold text-success flex items-center gap-1">
                            {row.users > 0 ? "▲ New Growth" : "--"}
                          </span>
                        </div>
                      </td>
                      <td className="bg-base-200/40 group-hover:bg-primary/10 text-right rounded-r-2xl pr-6 transition-colors">
                        <div className="badge badge-outline border-base-300 font-mono font-bold p-4 group-hover:border-primary/30 group-hover:bg-base-100 transition-all">
                          {row.messages.toLocaleString()}{" "}
                          <span className="ml-2 opacity-40 font-sans text-[10px]">
                            MSGS
                          </span>
                        </div>
                      </td>
            ...
            <div className="px-8 py-6 bg-base-200/50 border-t border-base-300 flex justify-between items-center">
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={handleGenerateReport}
                  className="btn btn-primary px-10 rounded-2xl shadow-xl shadow-primary/30 font-black"
                >
                  Export CSV
                </button>
              </div>
          ...
          <div
            className="modal-backdrop bg-base-900/60 backdrop-blur-md"
            onClick={() => setShowDetails(false)}
          ></div>
        </div>
      )}
...
const StatCard = ({ title, value, icon, loading, trend, isWarning }) => {
  if (loading)
    return (
      <div className="h-32 w-full bg-base-300 animate-pulse rounded-3xl"></div>
    );
    ...
    <div className="card bg-base-100 border border-base-300 shadow-sm hover:shadow-lg transition-all duration-300 group">
      <div className="card-body p-6">
        <div className="flex items-start justify-between">
          <div className="p-3 rounded-2xl bg-base-200/50 group-hover:scale-110 transition-transform">
            {icon}
          </div>
          {trend && (
            <span className="text-[10px] font-bold py-1 px-2 rounded-full bg-success/10 text-success">
              {trend}
            </span>
          )}
        </div>
        <div className="mt-4">
          ...
          <div className="flex items-baseline gap-2">
            <p
              className={`text-3xl font-black ${isWarning ? "text-error" : "text-base-content"}`}
            >
